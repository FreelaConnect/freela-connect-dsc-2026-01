...
1. `CI`: valida backend, build da imagem Docker de producao e publica a imagem no GitHub Container Registry em eventos controlados.
...
Validacoes principais nesta etapa:

- backend: `pnpm install --frozen-lockfile` e `pnpm run build`
- Docker: valida `docker-compose.prod.yml`, construi a imagem produtiva do backend e publica no GHCR quando o evento nao e `pull_request`

Proxima barreira recomendada: reativar `pnpm run lint:check`, testes unitarios e e2e depois de corrigir a divida existente da base atual.
