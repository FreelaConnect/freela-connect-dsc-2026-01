...
export class ContractTypeOrmRepository{
    ...
    async findAll(page: number = 1, limit: number = 10): Promise<{ data: ContractEntity[], total: number }> {
    ...
    async update(contractId: number, contractData: Partial<ContractEntity>): Promise<ContractEntity | null> {
        ...
        if (clientVersion !== undefined && clientVersion !== contract.version) {
            throw new ContractVersionConflictException(contractId, clientVersion, contract.version);
        }
    ...
    async delete(contractId: number, version?: number): Promise<boolean> {
        ...
        if (version !== undefined && version !== contract.version) {
            throw new ContractVersionConflictException(contractId, version, contract.version);
        }
