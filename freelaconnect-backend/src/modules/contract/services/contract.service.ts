...
export class ContractService {
    ...
    async confirmContract(contractId: number) : Promise<ConfirmContractResponseDto> {
    ...
    async getAllContracts(page: number = 1, limit: number = 10): Promise<any> {
        ...
        const mappedData = data.map(contract => new ContractResponseDto({
            contractId: contract.contractId,
            freelancerId: contract.freelancerId,
            orderId: contract.orderId,
            projectId: contract.projectId,
    ...
    async updateContract(contractId: number, updateContractDto: UpdateContractDto): Promise<ContractResponseDto> {
        const contract = await this.contractRepository.findById(contractId);
        if (!contract) {
            throw new ContractNotFoundException(contractId);
        }

        const updatedContract = await this.contractRepository.update(contractId, updateContractDto);
        if (!updatedContract) {
            throw new NotFoundException(`Falha ao atualizar contrato com ID ${contractId}`);
        }
    ...
    async replaceContract(contractId: number, replaceContractDto: ReplaceContractDto): Promise<ContractResponseDto> {
        const contract = await this.contractRepository.findById(contractId);
        if (!contract) {
            throw new ContractNotFoundException(contractId);
        }

        const replacedContract = await this.contractRepository.update(contractId, replaceContractDto);
        if (!replacedContract) {
            throw new NotFoundException(`Falha ao substituir contrato com ID ${contractId}`);
        }
    ...
    async deleteContract(contractId: number, version?: number): Promise<{ message: string }> {
        const contract = await this.contractRepository.findById(contractId);
        if (!contract) {
            throw new ContractNotFoundException(contractId);
        }
        ...
        if (!deleted) {
            throw new NotFoundException(`Falha ao deletar contrato com ID ${contractId}`);
        }
