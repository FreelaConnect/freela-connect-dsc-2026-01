  ...
  const createRepository = (): UsersRepository => {
    ...
    return {
      ...
      findByEmail: async (email) =>
        findVisibleUser((user) => user.email === email),
      findByPasswordResetTokenHash: async (passwordResetTokenHash) =>
        findVisibleUser(
          (user) => user.passwordResetTokenHash === passwordResetTokenHash,
        ),
      findAll: async (page = 1, limit = 10) => {
        const data = Array.from(users.values()).filter(
          (user) => !user.deletedAt,
        );
