...
export class PaymentService {
  ...
  async updatePayment(
    ...
    if (!updatedPayment) {
      throw new NotFoundException(
        `Falha ao atualizar pagamento com ID ${paymentId}`,
      );
    }
  ...
  async replacePayment(
    ...
    const replacedPayment = await this.paymentRepository.update(
      paymentId,
      updatePaymentDto,
    );
    if (!replacedPayment) {
      throw new NotFoundException(
        `Falha ao substituir pagamento com ID ${paymentId}`,
      );
    }
  ...
  async deletePayment(
    ...
    if (!deleted) {
      throw new NotFoundException(
        `Falha ao deletar pagamento com ID ${paymentId}`,
      );
    }
