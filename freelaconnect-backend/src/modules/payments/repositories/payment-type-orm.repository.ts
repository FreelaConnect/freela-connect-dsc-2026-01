...
export class PaymentTypeOrmRepository {
    ...
    async findAll(page: number = 1, limit: number = 10): Promise<{ data: PaymentEntity[], total: number }> {
    ...
    async update(paymentId: string, paymentData: Partial<PaymentEntity>): Promise<PaymentEntity | null> {
        ...
        if (clientVersion !== undefined && clientVersion !== payment.version) {
            throw new PaymentVersionConflictException(paymentId, clientVersion, payment.version);
        }
    ...
    async delete(paymentId: string, version?: number): Promise<boolean> {
        ...
        if (version !== undefined && version !== payment.version) {
            throw new PaymentVersionConflictException(paymentId, version, payment.version);
        }
